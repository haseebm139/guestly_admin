    ...
    <div class="card custom-card shadow-lg">
        <div class="card-body p-4 p-md-5">
            <div class="d-flex align-items-center mb-4">
                <!-- Profile Image -->
                <img src="{{ asset('avatar/default.png') }}" class="rounded-circle me-3" alt="User Image" width="60"
                    height="60">

                <!-- Name + Email -->
                <div>
                    <h5 class="mb-1 fw-bold">{{ $booking->client->name ?? 'Guest User' }}</h5>
                    <p class="mb-0 text-muted">{{ $booking->client->email ?? 'no-email@example.com' }}</p>
                </div>
            </div>
            ...
            <div class="tab-content" id="bookingTabsContent">
                ...
                <div class="tab-pane fade show active" id="appointment" role="tabpanel">
                    <div class="card shadow-sm">
                        ...
                        <div class="card-body">
                            <div class="mb-3">
                                <label class="form-label">Artist Identity</label>
                                <input type="text" class="form-control"
                                    value="
                                    @if (($booking->artist && $booking->artist->phone_verified == 1) || $booking->artist->email_verified == 1) Verified
                                    @else
                                    Unverified @endif"
                                    readonly>
                            </div>
                            <div class="mb-3">
                                ...
                                <input type="date" class="form-control" value="{{ $booking->booking_date }}"
                                    readonly>
                            </div>
                            <div class="mb-3">
                                <label class="form-label">Booking Time</label>
                                <input type="time" class="form-control" value="{{ $booking->booking_time }}"
                                    readonly>
                            </div>
                ...
                <div class="tab-pane fade" id="messages" role="tabpanel">
                    <div class="card shadow-sm h-100 d-flex flex-column">
                        ...
                        <div class="card-footer">
                            <form method="POST" {{-- action="{{ route('client.booking.message', $booking->id) }}" --}}>
                                @csrf
